...
The reason why usinng \emph{collapse} is making no difference relies on the number of iterations of the nested loops, in other words, the size of the image. Collpase makes sense when the outer loop has fewer iterations than threads available (so it prevetns any thread from being iddle) or when the work load is unevem among iterations.
\end{enumerate}
