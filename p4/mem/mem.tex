...
However, the difference between versions \emph{par0} and \emph{par2} is not that clear. 

At first, we may think that \emph{par2} should be better, because it uses a openmp caluse that is speciffic to this type of problem. But perhaps, the improvement of this clause can only be seen for very high sizes, and it is possible that the tested images are not big enough. This situation is similar to the one in exercise 2, where for some sizes it was better not to parallelize; in this case, for some sizes is may be better not to make use of this clause.
    \end{enumerate}
