...
For obtaining the results, it is necessary to run the simulation several times in order to calculate a mean and que no !!!!!!!dependa de un momento concreto!!!!!!!!. Furthermore, the chosen execution pattern alternates between the slow and fast version to avoid the program to reuse the cache from the previous execution (with the same size).
