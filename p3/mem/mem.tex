...
The first thing we notice is that as we increase the size of the cache, the lines for each block size configuration get closer. Therefore, it makes sense to think that the impact of the block size will be greater when the size of the matrix is of the order of the cache.

Let's analyze in detail the case of a 2048B cache. To determine the number of lines in the cache we have to do the following calculation: 
\[
\text{Number of rows} = \frac{\text{Cache Size}}{\text{Block Size}}
\]

If we subsitute for the different values of the block size, we get these number of lines.
